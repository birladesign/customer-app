...
// More than four dots and the labels collide into an unreadable wall on a
// phone-width strip, so a longer journey is trimmed to its most recent four
// stages wherever it's rendered. Full, un-elided history stays available in
// the Tracking Updates sheet.
const MAX_STEPS = 4;
...
export default function Tracker({ steps, currentIndex, stepIcons }) {
  const trimmed = Math.max(0, steps.length - MAX_STEPS);
  const visibleSteps = trimmed ? steps.slice(trimmed) : steps;
  const visibleCurrentIndex = currentIndex - trimmed;
  ...
  return (
    <div className={`tracker${stepIcons ? ' tracker--icons' : ''}`}>
      {visibleSteps.map((step, i) => {
        ...
        const prevDone = i > 0 && i - 1 < visibleCurrentIndex;
        ...
        return (
          <div key={label} className={`tracker__step tracker__step--${state}`}>
            <div className="tracker__rail">
              ...
              <span
                className={`tracker__line tracker__line--right${i === visibleSteps.length - 1 ? ' tracker__line--hidden' : ''}${state === 'done' ? ' tracker__line--done' : ''}`}
              />
            </div>
